        ...
        const questions: QuizQuestion[] = parsed.quiz.map(
            (q: unknown, index: number) => {
                ...
                for (let i = 0; i < question.options.length; i++) {
                    if (
                        question.options[i]?.toString().trim().toLowerCase() ===
                        answerText
                    ) {
                        correctAnswer = i;
                        break;
...
async function generateQuizName(
    topic?: string,
    ...
    try {
        const prompt = topic
            ...
            : `Generate a short, descriptive name (max 6 words) for a quiz based on these questions: ${quizQuestions
                  ?.slice(0, 2)
                  .map((q) => q.question)
                  .join(", ")}. Return only the name, no explanation.`;
            ...
            await tx
                ...
                .where(
                    and(
                        ...
                        eq(quizTable.userId, session.user.id),
                    ),
                );
        });
